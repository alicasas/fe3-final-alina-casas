...
const Card = ({ item }) => {
  ...
  const addFav = () => {
    const dentistAlreadyExists = dentistsStates.favs.find(
      (element) => element.id === item.id
    );
    !dentistAlreadyExists &&
      dentistsDispatch({ type: 'ADD_FAV', payload: item });
  };
  return (
    <>
      <Link className="card" to={`${routes.dentist}/${item.id}`}>
        <img
